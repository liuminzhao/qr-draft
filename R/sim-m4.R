#!/bin/Rscript
##' Time-stamp: <liuminzhao 09/03/2013 21:37:47>
##' 2013/08/31 simulation M4

sink('sim-m4-0903.txt')
rm(list = ls())
library(bqrpt)
library(quantreg)
library(xtable)
...
write.table(result, file="sim-m4-result-0903.txt", row.names = F, col.names = F)
...
## TRUE VALUE
###############
result <- read.table('sim-m4-result-0903.txt')
pMN.5 <- function(x){
