#!/bin/Rscript
##' Time-stamp: <liuminzhao 04/22/2014 00:52:41>
##' 2013/08/31 simulation M4H

sink('sim-m4h-0422.txt')
rm(list = ls())
library(bqrpt)
library(quantreg)
library(xtable)
...
write.table(result, file="sim-m4h-result-0422.txt", row.names = F, col.names = F)
...
## TRUE VALUE
###############
result <- read.table('sim-m4h-result-0422.txt')
pMN.5 <- function(x){
