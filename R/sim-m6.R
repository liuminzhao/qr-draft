#!/bin/Rscript
##' Time-stamp: <liuminzhao 09/01/2013 15:54:08>
##' 2013/08/31 simulation M6: t_3 error

sink('sim-m6-0901.txt')
rm(list = ls())
library(bqrpt)
library(quantreg)
library(xtable)
...
write.table(result, file="sim-m6-result-0901.txt", row.names = F, col.names = F)
sendEmail(subject = "simulation-m6", text = "done", address = "liuminzhao@gmail.com")
...
## TRUE VALUE
###############
result <- read.table('sim-m6-result-0901.txt')
truebetatau5 <- c(1,1,1)
truebetatau9 <- c(1+qt(0.9, df = 3),1,1)
