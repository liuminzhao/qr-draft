...
## test on M1, M2, M3, M4, to see which shows the advantage best

sink('sim-shrinkage-1008.txt')
rm(list = ls())
library(bqrpt)
library(quantreg)
library(xtable)
...
write.table(result, file="sim-shrinkage-result-1008.txt", row.names = F, col.names = F)
...
## TRUE VALUE
###############
result <- read.table('sim-shrinkage-result-1008.txt')
truebetatau5 <- c(1, 1, 0)
truebetatau9 <- c(1+qnorm(0.9),1, 0)
