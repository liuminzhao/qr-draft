#!/bin/Rscript
##' Time-stamp: <liuminzhao 09/04/2013 00:20:32>
##' 2013/08/31 simulation M1H normal
##' 2013/09/03 new

sink('sim-m1h-0903.txt')
rm(list = ls())
library(bqrpt)
library(quantreg)
library(xtable)
...
write.table(result, file="sim-m1h-result-0903.txt", row.names = F, col.names = F)
sendEmail(subject = "simulation-m1h", text = "done", address = "liuminzhao@gmail.com")

###############
## TRUE VALUE
###############
result <- read.table('sim-m1h-result-0903.txt')
truebetatau5 <- c(1,1)
