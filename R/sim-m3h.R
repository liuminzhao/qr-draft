#!/bin/Rscript
##' Time-stamp: <liuminzhao 09/06/2013 12:00:04>
##' 2013/08/31 simulation M3

sink('sim-m3h-0903.txt')
rm(list = ls())
library(bqrpt)
library(quantreg)
library(xtable)
...
write.table(result, file="sim-m3h-result-0903.txt", row.names = F, col.names = F)
...
## TRUE VALUE
###############
result <- read.table('sim-m3h-result-0903.txt')
truebetatau5 <- c(1,1)
