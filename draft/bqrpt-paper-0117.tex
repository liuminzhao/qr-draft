...
{\bf need to more explicitly differentiate what is in these papers from what we are doing} In order to solve those problems, simultaneous linear quantile
...
\end{align*}
We consider two choices for the priors for the parameters $(\bm{\beta}, \bm{\gamma})$.  First, we consider diffuse
p-dimensional normal distributions. {\bf minzhao: can you prove that if we use a flat (improper) prior that the posterior will be proper; this would be a nice result here}
Second, we consider spike and slab priors on ($\bm \beta,
\bm \gamma$) are an alternative that can do shrinkage and/or variable selection on both
quantile regression parameters and heterogeneity parameters and
improve efficiency; note that we keep the covariates the same for both to facilitate the form of the regression coefficients in (\ref{ch2:eq:newquan}).
For the latter,
we specify continuous spike and slab priors
\citep{george1993}
on $(\bm \beta, \bm
...
To optimize mixing, we adjust the acceptance rate of the
adaptive Metropolis-Hasting algorithm to around 0.2.  Tuning parameters are increased(decreased) by
...
In model 5, the heterogeneity comes from the mixture ofdistributions.
...
{\bf did some edits here to try to explain better.  make sure i didn't say anything wrong}
...
and covariates; both RQ and FBQR perform less well than PT/PTSS when the quantile of interest is not near the mode of the error. FBQR outperforms PT in some cases due to the flexibility
of the distribution of the error being
...
\begin{table}[h]
  \caption[]{\label{ch2:tab:tours} {\bf just PT and PTSS here.  also, in table if two significant digits to the left of the decimal, only use one digit to the right of the decimal} 95\% credible (confidence) intervals for
